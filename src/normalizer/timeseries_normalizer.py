...
class TimeseriesNormalizer:
    ...
    async def normalize(self, input_json: str) -> TurbinePowerDataPoints:
    ...
    async def calculate_bucket_value(self, bucket):
        ...
        for i in range(len(bucket) -1): # iterate over all but the last element (so we always have a next element)
            ...
            next_itme_minutes = next_measure_time.minute if next_measure_time.minute != 0 else 60
            value_weight = (next_itme_minutes - curr_measure_time.minute) / 30
